...
def Reconstruction_new_given(coeff,facemodel,tex2_path):
	id_coeff,ex_coeff,tex_coeff,angles,gamma,translation = Split_coeff(coeff)
	# compute face shape
	face_shape = Shape_formation(id_coeff, ex_coeff, facemodel)
	...
	# compute vertex color using SH function lighting approximation
	face_color,lighting = Illumination_layer(face_texture2, face_norm_r, gamma)

	# vertex index for each face of BFM model
	tri = facemodel.tri

	return face_shape,face_texture2,face_color,tri,face_projection,z_buffer,landmarks_2d
...
def Project_layer(face_shape):
	...
	#pdb.set_trace()
	clip_space_vertices = mesh_renderer.clip_vertices(face_shape,
